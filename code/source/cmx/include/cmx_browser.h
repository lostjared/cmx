	...
	namespace debug {
		...
		public:
			...
			void movementOutward() {
				...
				if(dir_path == "/") { updateAtPath(); return; }

				if(dir_path.rfind("/") >= 0) {
					 std::string  temp_dir_path = dir_path.substr(0, dir_path.rfind("/"));
					 if(temp_dir_path.length()==0) temp_dir_path="/";
					 if(isDir(temp_dir_path)) setPath(temp_dir_path);
				}
			}
			std::string selectedFile() {
				if(select_var+dir_offset > (int)array.size() || select_var+dir_offset < 0) {
					return "/";
				}
				std::string name = array[select_var+dir_offset];
